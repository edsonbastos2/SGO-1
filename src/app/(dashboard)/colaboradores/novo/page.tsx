    ...
    <div className="p-5 md:p-7 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#7e9ab5]">Preencha os dados em cada aba</p>
        </div>
      </div>
      ...
      {/* Campos do formulário — SEM tag form aqui */}
      <div className="bg-[#0f1623] border border-[#1a2540] rounded-xl p-6 space-y-5">
        {/* ── Aba: Dados Pessoais ── */}
        {aba === 'pessoal' && (
          <>
            <Field label="Prestadora" required error={errors.prestadoraId?.message}>
              <select {...register('prestadoraId')} className={inputCls(!!errors.prestadoraId)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
                <Field label="Nome completo" required error={errors.nome?.message}>
                  <input {...register('nome')} placeholder="Nome completo do colaborador" className={inputCls(!!errors.nome)} />
                </Field>
              </div>
              ...
              <Field label="Data de Nascimento">
                <input {...register('dataNasc')} type="date" className={inputCls()} />
              </Field>
        ...
        {aba === 'profissional' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Matrícula" required error={errors.matricula?.message}>
              <input {...register('matricula')} placeholder="Ex: 00123" className={inputCls(!!errors.matricula)} />
            </Field>
            <Field label="Data de Admissão" required error={errors.dataAdmissao?.message}>
              <input {...register('dataAdmissao')} type="date" className={inputCls(!!errors.dataAdmissao)} />
            </Field>
            <Field label="Função">
            ...
            <Field label="Escala">
              <select {...register('escalaId')} className={inputCls()}>
                <option value="">Selecione...</option>
                {escalas.map((e) => <option key={e.id} value={e.id}>{e.nome} ({e.jornadaHoras}h)</option>)}
              </select>
            </Field>
          </div>
        )}
        ...
        {aba === 'bancario' && (
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <Field label="Agência">
                <input {...register('agencia')} placeholder="0000" className={inputCls()} />
              </Field>
              <div className="sm:col-span-2">
                <Field label="Conta Bancária">
                  <input {...register('contaBancaria')} placeholder="00000-0" className={inputCls()} />
                </Field>
              </div>
            </div>
            <div className="border-t border-[#1a2540] pt-5">
              <p className="text-[10px] font-mono font-medium text-[#445870] uppercase tracking-widest mb-4">Chave PIX</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                {watch('tipoPix') && (
                  <Field label="Chave PIX">
                    <input {...register('chavePix')} placeholder="Informe a chave" className={inputCls()} />
                  </Field>
                )}
              </div>
            </div>
        ...
        {aba === 'alocacao' && (
          <div className="space-y-5">
            <div className="p-4 rounded-lg bg-[#0a0f1a] border border-blue-500/20 text-xs text-blue-400">
              Opcional — você pode alocar o colaborador agora ou depois pela tela de Alocações.
            </div>
              ...
              <select {...register('postoId')} className={inputCls()}>
                ...
                {postos.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.tomador.nomeFantasia ?? p.tomador.razaoSocial} · {p.identificador}
                  </option>
                ))}
              </select>
            </Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <Field label="Turno">
                <select {...register('turnoId')} className={inputCls()}>
                  ...
                  {turnos.map((t) => <option key={t.id} value={t.id}>{t.nome} ({t.entrada}–{t.saida})</option>)}
                </select>
              </Field>
      ...
      {erro && (
        <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs">{erro}</div>
      )}

      {/* Navegação — FORA do form */}
      <div className="flex gap-3">
        {abaIdx === 0 ? (
          ...
          <button type="button" onClick={voltarAba}
